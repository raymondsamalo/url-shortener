...
from app.ui.delete_dialog import DeleteLinkDialog
...
async def main_page():
    ...
    table = ui.table(columns=columns, rows=initial_rows,
                     row_key='id').classes('w-full items-stretch')
    # rows_per_page = 2
    ...
    async def delete_data(form) -> DialogError | None:
        link_input = form.link_input.value
        try:
            await repo.delete_link(link=link_input)
            table.rows = await load_data(hostname=hostname)
            ui.notify(f'{link_input} deleted')
        except exc.OperationalError as e:
            logger.exception(e)
            return DialogError("Database error, please retry")
        except exc.SQLAlchemyError as e:
    ...
    def handle_delete(link: str, url: str):
        """Filters out the target row by its unique name and refreshes the table."""
        delete_dialog = DeleteLinkDialog(delete_data)
        delete_dialog.set_values(link=link, url=url)
        delete_dialog.show()

    def open_edit_dialog(link: str, url: str):
        edit_dialog = EditLinkDialog(edit_data)
        edit_dialog.set_values(link=link, url=url)
        edit_dialog.show()
        ...
        with table.cell('actions'):
            ...
            with ui.row().classes('items-center justify-center no-wrap gap-2'):
                ...
                ui.button(icon='delete').props('flat dense color=negative').on(
                    'click',
                    js_handler='() => emit(props.row.link,props.row.url)',
                    handler=lambda e: handle_delete(e.args[0], e.args[1])
                )

    with ui.header().classes('bg-blue-500 text-white p-4 items-center'):
        ui.label('Url Shortener').classes('text-h6')
        ui.space()
        ui.button(icon="sync", on_click=refresh_data).classes(
            'q-mt-md').props('flat dense color=white')
        ui.button(icon="settings", on_click=lambda: ui.navigate.to(
            '../docs')).classes('q-mt-md').props('flat dense color=white')

    new_dialog = NewLinkDialog(new_data)
    ui.button('Add', on_click=new_dialog.show).props(
        'fab icon=add').classes('fixed bottom-4 right-4')
